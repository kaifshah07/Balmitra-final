...
export default function AdminFranchiseLeadsPage() {
  ...
  async function loadLeads() {
    try {
      setLoading(true);
      const res = await api.get("/franchise-enquiries");
  ...
  async function handleStatusChange(id, newStatus) {
    try {
      await api.patch(`/franchise-enquiries/${id}/status`, { status: newStatus });
      loadLeads();
  ...
  async function handleDelete(id) {
    ...
    try {
      await api.delete(`/franchise-enquiries/${id}`);
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Franchise Leads</h1>
          <p className="text-sm text-gray-500 mt-1">
            Store franchise applications and store expansion inquiries
          </p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b text-xs uppercase tracking-wider text-gray-600">
                ...
                <th className="p-4">Store Type</th>
              ...
              {leads.map((lead) => (
                <tr key={lead.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="p-4 text-gray-700">
                    {lead.preferredCity || lead.city}, {lead.state}
                  </td>
                  <td className="p-4 text-gray-700 font-medium">{lead.storeType || "-"}</td>
                  <td className="p-4 text-gray-600">{lead.investmentCapacity || "-"}</td>
                  <td className="p-4 text-gray-500 text-xs">{lead.startTimeline || "-"}</td>
                  <td className="p-4">
                    <select
                      value={lead.status}
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 shadow-xl">
            ...
            <div className="space-y-3 text-sm">
              <div>
                ...
                <span className="font-bold text-gray-800">{selectedLead.fullName}</span>
              </div>
              <div>
              ...
              <div>
                <span className="font-semibold text-gray-600">Preferred Store Location:</span>{" "}
                {selectedLead.preferredCity} ({selectedLead.preferredLocation})
              </div>
              <div>
                <span className="font-semibold text-gray-600">Store Type:</span>{" "}
                {selectedLead.storeType || "Not specified"}
              </div>
              <div>
                <span className="font-semibold text-gray-600">Investment Budget:</span>{" "}
                {selectedLead.investmentCapacity}
              </div>
              <div>
                <span className="font-semibold text-gray-600">Timeline to Start:</span>{" "}
